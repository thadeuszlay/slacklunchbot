...
module.exports = function (param) {
	...
	request(endpoint, function (err, response, body) {
		...
		var randomIndex = Math.floor((Math.random() * lunchLocation.length) + 1) -
			1;
		var lunchSuggestion = lunchLocation[randomIndex];
		info.push('Let\'s go to the ' + lunchSuggestion.name + '. ' +
			lunchSuggestion.text + go1 + lunchSuggestion.duration + go2);
